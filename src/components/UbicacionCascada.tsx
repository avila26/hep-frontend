...
export interface UbicacionCascadaProps {
    ...
    error?: string;
    disabled?: boolean;
    /** 'grid' renders the 4 dropdowns in a 2×2 grid instead of stacked vertically */
...
export const UbicacionCascada: React.FC<UbicacionCascadaProps> = ({
    ...
    const lastSentValue = useRef<string | null>(null);
    ...
    const handleAmbienteChange = (e: DropdownChangeEvent) => {
        ...
        if (bloqueActual && pisoActual && servicioActual && ambiente) {
            triggerChange(`${bloqueActual.label} > ${pisoActual.label} > ${servicioActual.label} > ${ambiente}`);
        }
    };
    ...
    const locationSummary = value && (
        <div className="mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm text-slate-600 flex items-center gap-2">
